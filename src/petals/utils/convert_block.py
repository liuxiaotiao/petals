...
def convert_block(
    ...
    **kwargs,
    ...
    if getattr(config, "petals_custom_cache", False):
        if len(tensor_parallel_devices) != 1 or quant_type != QuantType.NONE or adapters:
            raise ValueError("Qwen currently requires one device per server, --quant_type none, and no adapters")
...
def make_tensor_parallel(
    ...
    if getattr(model_config, "petals_custom_cache", False):
        if len(devices) != 1:
            raise ValueError("Qwen requires one device per server; distribute layers across separate servers")
        # Explicit empty rules preserve packed expert tensors and hybrid cache shapes.
        return tp.TensorParallel(
            block, devices, config=tp.Config({}, {}, {}, {}), output_device=output_device, delay_init=True
        )
