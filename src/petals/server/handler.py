...
class TransformerConnectionHandler(ConnectionHandler):
    ...
    def _reject_if_inference_only(self, method: str) -> None:
        """Training is the only thing rpc_backward is used for, so this is where we draw the line."""
        if self.inference_only:
            raise RuntimeError(
                f"This server runs with --inference_only and does not serve {method}. "
                f"It accepts rpc_inference and rpc_forward; training is not available in this swarm"
            )

    async def rpc_backward(self, request: runtime_pb2.ExpertRequest, context: P2PContext) -> runtime_pb2.ExpertResponse:
        self._reject_if_inference_only("rpc_backward")
        async with timeout(self.request_timeout):
            # Parse requests and prepare backends
    ...
    async def rpc_backward_stream(
        ...
        self._reject_if_inference_only("rpc_backward_stream")
        async with timeout(self.request_timeout):
    ...
    async def rpc_info(self, request: runtime_pb2.ExpertUID, context: P2PContext) -> runtime_pb2.ExpertInfo:
        ...
        result = {
            ...
            CACHE_TOKENS_AVAILABLE: max(0, backend.memory_cache.bytes_left - backend.cache_fixed_bytes)
            // max(backend.cache_bytes_per_token.values()),
        }
