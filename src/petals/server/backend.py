...
class TransformerBackend(ModuleBackend):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not getattr(config, "petals_custom_cache", False):
        ...
        self.custom_cache = getattr(config, "petals_custom_cache", False)
        ...
        if self.custom_cache:
            assert len(self.module.devices) == 1
        ...
        self.cache_fixed_bytes = 0
        if self.custom_cache:
            # Discovery reports an estimate; MemoryCache still enforces exact per-session sizes.
            one = sum(self.cache_bytes_per_token.values())
            two = sum(
                d.numel() * get_size_in_bytes(d.dtype)
                for d in self.get_inference_cache_descriptors(batch_size=1, max_length=2)
            )
            self.cache_bytes_per_token = {self.module.devices[0]: two - one}
            self.cache_fixed_bytes = 2 * one - two
    ...
    def get_inference_cache_descriptors(self, batch_size: int, max_length: int) -> Sequence[TensorDescriptor]:
        ...
        if self.custom_cache:
            ...
            return [
                ...
                for shape, dtype in cache_specs(self.config, block.layer_type, batch_size, max_length, self.dtype)
            ]
    ...
    def forward(self, *inputs: Union[torch.Tensor, str]) -> Tuple[torch.Tensor, ...]:
        *inputs, active_adapter = inputs
        with self._using_adapter(active_adapter):
    ...
    def _using_adapter(self, active_adapter):
        if self.custom_cache:
            if active_adapter:
                raise ValueError("Adapters are not supported by the experimental Qwen backend")
            return nullcontext()
        return self._peft_module.using_adapter(active_adapter)
    ...
    def inference_step(
        ...
        with self.memory_cache.use_cache(*inference_info.cache_handles) as cache_tensors, self._using_adapter(
            inference_info.active_adapter
        ):
            self._reorder_cache_inplace(cache_tensors, hypo_ids)

            if self.custom_cache:
                chunk_length = min(256, self._estimate_max_chunk_length(hidden_states, inference_info))
                return self.module.module_shards[0].inference_with_cache(
                    hidden_states, cache_tensors, inference_info.prefix_length, chunk_length
                )
    ...
    def get_pools(self) -> Sequence[PrioritizedTaskPool]:
        if self.inference_only:
            # The backward pool would sit idle: the handler refuses rpc_backward in this mode.
