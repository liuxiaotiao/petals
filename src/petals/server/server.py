    ...
    def __init__(
        ...
        **kwargs,
        ...
        if quant_type is None:
            quant_type = (
                QuantType.NF4
                if device.type == "cuda" and not getattr(self.block_config, "petals_custom_cache", False)
                else QuantType.NONE
            )
        if getattr(self.block_config, "petals_custom_cache", False):
            if quant_type != QuantType.NONE or len(self.tensor_parallel_devices) != 1 or adapters:
                raise ValueError("Qwen requires --quant_type none, one device per server, and no adapters")
            logger.warning("Experimental Qwen text adapter: eager kernels, no quantization, TP, vision, or MTP")
        ...
        if getattr(self.block_config, "petals_custom_cache", False):
            ...
            self._cache_bytes_per_block = max(
                sum(
                    math.prod(shape) * get_size_in_bytes(dtype)
                    for shape, dtype in cache_specs(self.block_config, kind, 1, attn_cache_tokens, self.torch_dtype)
                )
                for kind in set(self.block_config.layer_types)
            )
        ...
        if inference_only:
            logger.info("Running in inference-only mode: rpc_backward is disabled, so this server serves no training")
